...
import { getSupremeCourtCasesPage } from '@/services/judiciary';

type SupremeCourtCasesClientProps = {
  initialClusters: Cluster[];
  judges: Judge[];
};

export default function SupremeCourtCasesClient({ initialClusters, judges }: SupremeCourtCasesClientProps) {
  ...
  const [selectedJudge, setSelectedJudge] = useState<Judge | null>(
    judges.find((judge) => String(judge.id) === currentJudgeId) ?? null,
  );
  ...
  const fetchClusters = useCallback(async (page: number) => {
    if (page === 1) {
      setClusters([]);
    }

    try {
      const data = await getSupremeCourtCasesPage({
  ...
  }, [judgeId, searchQuery, startDate, endDate, sortOrder]);
  ...
  useEffect(() => {
    ...
    router.push(`/supreme-court-cases${queryString ? `?${queryString}` : ''}`, { scroll: false });
    // resetScroll is intentionally omitted: the hook recreates it as pagination advances.
