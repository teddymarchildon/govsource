...
class Congressman(GovLensModel):
    ...
    )  # Changed to String to handle both integers and empty values
...
class BillText(GovLensModel):
    ...
    formatted_bill_id = Column(String(20), ForeignKey("bills.bill_id"), index=True)
    ...
    __table_args__ = (UniqueConstraint("formatted_bill_id", "type", name="uix_bill_type"),)
