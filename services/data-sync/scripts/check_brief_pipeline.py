...
def problems(overview, now=None):
    ...
    oldest=overview.get('oldest_current_waiting',overview.get('oldest_waiting'))
    ...
    progress=overview.get('last_progress_at')
    if overview.get('current_waiting',0) and (not progress or now-datetime.fromisoformat(progress.replace('Z','+00:00'))>timedelta(hours=6)):
